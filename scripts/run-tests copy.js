...
// the EspruinoTools Node API. Detects PASS/FAIL from console output.
...
Options:
  ...
  -q, --quiet   Suppress EspruinoTools serial debug logging
`);
}
...
`;
      return prologue + src + epilogue;
        ...
        async function runOneTest(E, port, testPath, timeoutMs=15000, quiet=false) {
  const fs = require('fs');
...
?
/).forEach(line=>{
        if (line.includes('__espruino_test__')) { try{ record=JSON.parse(line);}catch(e){} }
      });
...
async function main() {
  ...
  for (const t of tests) {
    ...
    try {
      const res = await runOneTest(E, port, t.path, 15000, Boolean(args.quiet));
      ...
      try {
        ...
        console.log(`
Saved results to results/${stamp}/${board}/`);
      } catch(e){}
