...
| Issue | Symptoms | Status | Fix Options |
|-------|----------|--------|-------------|
| **Issue 1 – Serial TX queue underflow** | `TypeError: ... txItem.data.length` during CLI upload | Implemented locally (pending upstream PR) | Guard in `EspruinoTools/core/serial.js` (Issue 1 diff). |
| **Issue 2 – CLI iterator crash** | Second CLI invocation fails with `port.type` error | Implemented locally (pending upstream PR) | Iterator rewrite in `EspruinoTools/bin/espruino-cli.js` (Issue 2 diff). |
| **Issue 3 – Post-reset prompt missing** | Harness waits for `__RUNNER_READY__` after `reset();` | Prompt retry patch applied locally | Keep harness newline (Issue 3A) and/or upstream patch (Issue 3B diff). |
| **Issue 4 – `ENV_ON_CONNECT` loop on Pico** | Warm-up spams “Unable to retrieve board information” | Harness disables `ENV_ON_CONNECT` | Optional future EspruinoTools tweak once Issue 3 lands. |
