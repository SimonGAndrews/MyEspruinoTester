...
(function(){
  try {
    ...
    if (ok) {
      ...
      __fail('Wifi module not available or unexpected type');
    }
  } catch (e) {
    __fail('require(\'Wifi\') threw: ' + ((e && e.message) || e));
  }
})();
```
...
- **`result` value:** Assigning `result` ends the test. You can use:
  ...
  - `__fail('reason')` or `result = { status: 'fail', reason: '...' }` for detailed outcomes.
  The runner understands the helper functions above, plain booleans, or objects with `status` and optional `reason` fields (legacy shapes such as `{ pass: true }` are still accepted for backward compatibility).
...
(function(){
  ...
  function done(status, ok, reason) {
    ...
    var payload = { __espruino_test__: true, file: "test_module_presence.js", status: status, duration_ms: Date.now()-__t0, reason: reason || null };
...
The wrapper exposes convenience helpers for signalling outcomes:
...
Each helper assigns the canonical result object `{ status, reason }`. You can still set `result = { status: 'pass', reason: '...' }` manually if you prefer—the helpers simply make the intent explicit. Legacy shapes (booleans, `{ pass: true }`, `{ skip: true }`) continue to work for backwards compatibility, but new tests should use the helpers so behaviour is unambiguous. Older examples elsewhere in this guide show the legacy pattern; interpret them through the lens of these helpers.
