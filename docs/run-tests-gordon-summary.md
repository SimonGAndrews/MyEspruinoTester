...
5) Session defaults from `EspruinoTools/configDefaults.json` are merged with board metadata, followed by optional CLI overrides (fixture injections, CLI flags), producing the base configuration used for every test.

6) Desired suites are resolved using `resolveSuites`, which compares the requested suite list (from `--suites` or metadata defaults) to the profile’s declared `suites.available`. Unknown suites produce an error.
...
- The effective configuration is assembled by cloning the base config and merging suite, test, and CLI layers. Loader delays (`preUploadDelayMs`, `postUploadDelayMs`, `timeoutMs`) are normalised, fixture data is prepared for injection, and storage preload directives are identified.
...
11) **Sleep delay handling (new):**  
    ...
    - When set—and when an explicit `--sleep` is not already present—the CLI command is augmented with `--sleep <ceil(ms/1000)>`.  
