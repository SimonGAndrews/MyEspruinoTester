...
|---------|--------------|------------|
| `Error: board manifest not found` | Typo or missing `boards/<name>.json` | Check the board filename/case. For community boards, generate a JSON from the firmware repo with `python scripts/build_board_json.py boards/<NAME>.py > boards/<NAME>.json` and point the harness at it with `--board`. |
